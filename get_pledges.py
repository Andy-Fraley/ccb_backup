...
login_request = {
    'ax': 'login',
    'rurl': '/index.php',
    'form[login]': '', # Source from util/settings
    'form[password]': '' # Source from util/settings
}
...
with requests.Session() as http_session:
    ...
    if login_response.status_code == 200:
        match_login_info = re.search('individual: {\s+id: 5,\s+name: "Andy Fraley"', login_response.text)
    ...
    if not pledge_summary_succeeded:
        print 'Pledge Summary retrieval failure!'
        sys.exit(1)
    ...
    if report_page.status_code == 200:
        ...
        print 'Error retrieving report settings page!'
        sys.exit(1)

    dict_pledge_categories = {}
    root_str = ''
    for option_match in re.finditer(r'<option\s+value=\\"([0-9]+)\\"\s*>([^<]*)<\\/option>', pledge_categories_str):
    ...
    for pledge_category in list_pledge_categories:
        if pledge_category in dict_pledge_categories:
            ...
            print 'Error!  Unknown pledge category. ' + pledge_category
